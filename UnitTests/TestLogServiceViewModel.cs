...
public class TestLogServiceViewModel
{
    ...
    /// <summary>
    /// Test the IsEnabled property and its impact on dependent properties.
    /// </summary>
    [TestMethod]
    public void TestIsEnabledDependentProperties()
    {
        // Act
        _viewModel.IsEnabled = true;

        // Assert
        Assert.AreEqual(Visibility.Visible, _viewModel.UploadAndCloudSyncButtonsVisibility,
            "UploadAndCloudSyncButtonsVisibility should be Visible when IsEnabled is true.");
        Assert.AreEqual(Visibility.Collapsed, _viewModel.SyncUpButtonVisibility,
            "SyncUpButtonVisibility should be Collapsed when IsEnabled is true.");

        // Act
        _viewModel.IsEnabled = false;

        // Assert
        Assert.AreEqual(Visibility.Collapsed, _viewModel.UploadAndCloudSyncButtonsVisibility,
            "UploadAndCloudSyncButtonsVisibility should be Collapsed when IsEnabled is false.");
        Assert.AreEqual(Visibility.Visible, _viewModel.SyncUpButtonVisibility,
            "SyncUpButtonVisibility should be Visible when IsEnabled is false.");
    }
